...
* *Compatibilidad con Node.js*: v25.x
* *Sistema operativo*:
+
** CentOS 8.0 y 8.5
+
...
+
Un sistema Red Hat Enterprise Linux debe estar registrado en Red Hat Subscription Management. Si no está registrado, el sistema no puede acceder a los repositorios para actualizar el software de terceros necesario durante la instalación.
...
. En la página *Definir relación de sincronización*, elige un origen y un destino y luego selecciona *Continuar*.
+
...
. Introduce un nombre para el agente de datos y selecciona *Continuar*.
+
Después de que se cargue la página de instrucciones, sigue las instrucciones que incluyen un enlace único para descargar el instalador.

. En la página de instrucciones:
+
.. Elige habilitar el acceso a *AWS*, *Google Cloud* o ambos.
